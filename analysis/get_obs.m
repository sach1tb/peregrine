...
%         end
        ...
        title('tail (darker is later in time)')
        
        % raw and detrended value
        subplot(1,3,2); gca;
        plot((1:numel(a1))/fps, tey1, 'b', 'linewidth', 3);
        hold on;
        plot((1:numel(a1))/fps, ey1, 'r', 'linewidth', 3);
        box off;
        set(gca, 'fontsize', 24);
        set(gca, 'ylim', [xx/2 -xx/2]);
        ylabel('tail-tip (pixels)')
        ...
        if 0 % to store as a movie
            set(gcf, 'paperpositionmode', 'auto');
            print('-dpng', sprintf('/tmp/frm_%.5d.png', dd));
        end
    end
...
for ii =1:size(FileName,2)
    ...
    TE=transfer_entropy(X, Y, 1, ds, nb, labels, 0); % show=0
    dat(ii,:)=TE.XY;
end
...
function TE=transfer_entropy(X,Y,k,ds, nb, labels, show)
...
r=nb;
TE.YX=tehist(X,Y,k,ds, r);
TE.XY=tehist(Y,X,k,ds, r);
TE.r=r;
